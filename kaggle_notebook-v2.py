...
for idx, (group_name, r) in enumerate(results.items()):
    ...
    # Build features with proper encoders
    ...
    # Save model + encoders for later use
    saved_models[group_name] = {
        "model": model,
        "features": cols,
        "params": r['kept'],
        "le_etendue": le_et,
...
# 
# Save all 5 models + encoders to a single file.
# This file will appear in the notebook's **Output** tab.
# Other notebooks can load it via **Add Data → Your Work → this notebook**.

# %%
# Save to /kaggle/working/ (notebook output)
import os

save_path = "/kaggle/working/eie_models.joblib"

# If running locally, save to current directory instead
if not os.path.exists("/kaggle/working"):
    save_path = "eie_models.joblib"

joblib.dump(saved_models, save_path)
size_kb = os.path.getsize(save_path) / 1024
print(f"💾 Saved: {save_path} ({size_kb:.0f} KB)")
print(f"   Contains: {len(saved_models)} XGBoost models + encoders")
print(f"   Groups: {list(saved_models.keys())}")
print(f"\n   To load in another notebook:")
print(f"   models = joblib.load('/kaggle/input/YOUR-NOTEBOOK-SLUG/eie_models.joblib')")
...
# 
# **Saved model**: `eie_models.joblib` — use it in a predictor notebook without retraining.
